...
M.open = function(opts)
  ...
  if state.bufnr == nil or is_new then
    ...
    if state.query.base_path ~= nil
        and #state.query.base_path > 1
        and opts.base_path == '.'
    then
      opts.base_path = state.query.base_path
    end
  end
...
M.on_search_change = function()
  ...
  for index, line in pairs(lines) do
    ...
    if index >= 9 and index <= 11 and #line > 0 then
      query.base_path = query.base_path .. line
    end
  end
  local line = vim.fn.getpos('.')
...
M.resume_last_search = function()
  ...
  ui.render_text_query({
    ...
    base_path = state.query_backup.base_path
  })
  ui.render_search_ui()
...
M.search = function(opts)
  ...
  state.finder_instance:search({
    ...
    base_path = state.query.base_path,
  })
  M.init_regex()
end
