...
function M.render_search_ui()
  ...
  local base_path_message = "Base Path:"
  if state.cwd then
    base_path_message = base_path_message .. string.format("   cwd=%s", state.cwd)
  end

  table.insert(details_ui, { { base_path_message, state.user_config.highlight.ui } })
  ...
  print(" details_ui:", vim.inspect(details_ui)) -- __AUTO_GENERATED_PRINT_VAR__
end
...
M.render_text_query = function(opts)
  ...
  api.nvim_buf_set_lines(state.bufnr, 8, 8, false, { opts.base_path })
